...
// =====================================================
// GOVERNED SPECIAL REQUIREMENT UPDATE MUTATION
// =====================================================

if ($type === 'applicationSpecialRequirementUpdate') {
    // Resolve authoritative identifiers
    $applicationId = (int)(
        $input['applicationID'] ?? 0
    );
    ...
    // Return governed mutation error
    $returnRequirementUpdateError = static function (
        string $message
    ): never {
        echo json_encode([
            'success' => false,
            'type' =>
                'application_special_requirement_update',
            'error' => $message
        ], JSON_UNESCAPED_SLASHES);
        exit;
    };
    ...
    if (
        ...
        $returnRequirementUpdateError(
            'A valid Application, Special Requirement, and authenticated Contact are required.'
        );
    }

    // Resolve submitted values
    $statusId = (int)(
        $requirementInput['statusID'] ?? 0
    );
    ...
    try {
        ...
        // Load authoritative Special Requirement
        $requirementStmt = $db->prepare("
            SELECT
                r.*,
                s.applicationSpecialRequirementStatusName,
            ...
            INNER JOIN tblApplicationSpecialRequirementStatuses s
                ON s.applicationSpecialRequirementStatusID =
                   r.applicationSpecialRequirementStatusID
            WHERE r.applicationSpecialRequirementID =
                :requirementId
        ...
        $requirementBefore = $requirementStmt->fetch(
            PDO::FETCH_ASSOC
        );

        if (!is_array($requirementBefore)) {
            $returnRequirementUpdateError(
                'Special Requirement was not found.'
            );
        }

        // Validate active destination status
        $statusStmt = $db->prepare("
            SELECT
                applicationSpecialRequirementStatusID,
                applicationSpecialRequirementStatusName,
        ...
        // Resolve authoritative completion timestamp
        $isSatisfied = strcasecmp(
        ...
        $completedUnix = $isSatisfied
            ? (
                $requirementBefore[
                    'applicationSpecialRequirementCompletedUnix'
                ] !== null
                    ? (int)$requirementBefore[
                        'applicationSpecialRequirementCompletedUnix'
                    ]
                    : $updatedUnix
            )
        ...
        // Resolve whether status changed
        $statusChanged =
            (int)$requirementBefore[
                'applicationSpecialRequirementStatusID'
            ] !== $statusId;
        ...
        $eventType = $statusChanged
            ? 'application.special_requirement.status_changed'
            : 'application.special_requirement.updated';

        // Begin atomic governed mutation
        $db->beginTransaction();

        // Update authoritative Special Requirement
        $updateStmt = $db->prepare("
            UPDATE tblApplicationSpecialRequirements
            SET
                applicationSpecialRequirementStatusID =
                    :statusId,
                applicationSpecialRequirementDescription =
        ...
        // Build authoritative before-and-after Event data
        $eventData = json_encode([
            'operation' =>
                'application.special_requirement.update',
            'applicationSpecialRequirementID' =>
                $requirementId,
            'before' => [
                'statusID' => (int)$requirementBefore[
                    'applicationSpecialRequirementStatusID'
                ],
                'statusName' => $requirementBefore[
                    'applicationSpecialRequirementStatusName'
                ],
                'description' => $requirementBefore[
                    'applicationSpecialRequirementDescription'
                ],
                'responsibleParty' => $requirementBefore[
                    'applicationSpecialRequirementResponsibleParty'
                ],
                'requiredUnix' => $normalizeUnix(
                    $requirementBefore[
        ...
        $actionId = insertActionPrompt([
            ...
            'activitySessionId' => $activitySessionId,
            'promptText' =>
                'Update Application Special Requirement',
            'responseText' => sprintf(
                'Updated Special Requirement #%d for Application #%d.',
                $requirementId,
                $applicationId
            ),
            'intent' => 'application.update',
        ...
        error_log(
            '[askOpenAI] Special Requirement update failed: ' .
            $e->getMessage()
        );
        ...
        echo json_encode([
            ...
            'type' =>
                'application_special_requirement_update',
            'error' =>
                'Unable to update the Special Requirement.'
        ], JSON_UNESCAPED_SLASHES);
        exit;
    }
}
...
// =====================================================
// GOVERNED APPLICATION FEE UPDATE MUTATION
// =====================================================

if ($type === 'applicationFeeUpdate') {
    // Resolve governed identifiers
    $applicationId = (int)(
        $input['applicationID'] ??
        0
    ...
    // Return governed Fee update errors
    $returnFeeUpdateError = static function (
        string $message
    ): never {
        echo json_encode([
            'success' => false,
            'type' => 'application_fee_update',
            'error' => $message
        ], JSON_UNESCAPED_SLASHES);
        exit;
    };
    ...
    if (
        ...
        $returnFeeUpdateError(
            'Fee Amount must be greater than zero and within the supported limit.'
        );
    }
    ...
    try {
        ...
        // Protect paid financial records
        if ($fee['feePaidUnix'] !== null) {
            $returnFeeUpdateError(
                'A paid Application Fee cannot be edited.'
            );
        }

        // Normalize authoritative before-values
        $before = [
            'feeCategory' =>
                (string)$fee['feeCategory'],
            'feeAmount' =>
                round(
                    (float)$fee['feeAmount'],
                    2
        ...
        // Reject mutations without authoritative changes
        $hasChanges =
            $before['feeCategory'] !==
                $after['feeCategory'] ||
            (int)round(
                $before['feeAmount'] * 100
            ) !==
                (int)round(
                    $after['feeAmount'] * 100
                ) ||
            $before['feeNote'] !==
                $after['feeNote'] ||
            $before['feeAssessedUnix'] !==
                $after['feeAssessedUnix'];

        if (!$hasChanges) {
            $returnFeeUpdateError(
                'No Application Fee changes were submitted.'
            );
        }

        // Begin governed Fee update transaction
        $db->beginTransaction();

        $updatedUnix = time();

        // Update outstanding authoritative Fee
        ...
        if ($updateFeeStmt->rowCount() !== 1) {
            throw new RuntimeException(
                'Application Fee could not be updated.'
            );
        }

        // Build authoritative Fee Event data
        $eventData = json_encode([
            'operation' =>
        ...
        $actionId = insertActionPrompt([
            ...
            'responseText' =>
                sprintf(
                    'Updated %s Fee #%d to $%s for Application #%d.',
                    $feeCategory,
                    $feeId,
                    number_format(
                        $feeAmount,
        ...
        error_log(
            '[askOpenAI] applicationFeeUpdate failed: ' .
            $e->getMessage()
        );
...
// =====================================================
// GOVERNED APPLICATION FEE PAYMENT MUTATION
// =====================================================

if ($type === 'applicationFeeMarkPaid') {
    // Resolve governed identifiers
    $applicationId = (int)(
        $input['applicationID'] ??
        0
    ...
    // Resolve Payment timestamp
    $feePaidUnix = is_numeric(
        $input['feePaidUnix'] ??
        null
    )
        ? (int)$input['feePaidUnix']
        : time();
    ...
    // Return governed Fee payment errors
    $returnFeePaymentError = static function (
        string $message
    ): never {
        echo json_encode([
            'success' => false,
    ...
    if ($feePaidUnix <= 0) {
        $returnFeePaymentError(
            'A valid Fee Paid date is required.'
        );
    }
    ...
    try {
        ...
        if ($fee['feePaidUnix'] !== null) {
            $returnFeePaymentError(
                'Application Fee is already marked paid.'
            );
        }
        ...
        // Begin governed Fee payment transaction
        $db->beginTransaction();

        $updatedUnix = time();

        // Mark authoritative Fee paid
        $updateFeeStmt = $db->prepare("
            UPDATE tblApplicationFees
            SET feePaidUnix = :feePaidUnix
            WHERE feeID = :feeId
              AND applicationID = :applicationId
              AND feePaidUnix IS NULL
            LIMIT 1
        ...
        if ($updateFeeStmt->rowCount() !== 1) {
            throw new RuntimeException(
                'Application Fee payment status could not be updated.'
            );
        }

        // Build authoritative Payment Event data
        $eventData = json_encode([
            'operation' =>
                'application.fee.mark_paid',
            'feeID' =>
                $feeId,
            'feeCategory' =>
                $feeCategory,
        ...
        if ($eventData === false) {
            throw new RuntimeException(
                'Application Fee Payment Event data could not be encoded.'
            );
        }
        ...
        if ($applicationEventId <= 0) {
            throw new RuntimeException(
                'Application Fee Payment Event could not be created.'
            );
        }
        ...
        $actionId = insertActionPrompt([
            ...
            'activitySessionId' =>
                $activitySessionId,
            'promptText' =>
                'Mark Application Fee Paid',
            'responseText' =>
                sprintf(
                    'Marked %s Fee #%d of $%s paid for Application #%d.',
                    $feeCategory,
                    $feeId,
                    number_format(
                        $feeAmount,
        ...
        if ($actionId <= 0) {
            throw new RuntimeException(
                'Application Fee Payment Action could not be created.'
            );
        }
        ...
        // Roll back incomplete Fee payment
        if ($db->inTransaction()) {
            $db->rollBack();
        }

        error_log(
            '[askOpenAI] applicationFeeMarkPaid failed: ' .
            $e->getMessage()
        );

        $returnFeePaymentError(
            'Unable to mark the Application Fee paid.'
        );
    }
}
