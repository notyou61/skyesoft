<?php
// /skyesoft/api/addAction.php — log an action with geo + optional Place ID, PHP 5.x safe
...
// Place ID lookup (graceful via public endpoint)
$placeId = null;
if (isset($body['actionLatitude']) && isset($body['actionLongitude'])) {
  ...
  if (function_exists('curl_init')) {
    ...
    if ($resp === false) $cerr = 'HTTP request failed';
  }
  if (!$cerr && is_string($resp) && strlen($resp)>0) {
    $pd = json_decode($resp, true);
    if (is_array($pd)) $placeId = isset($pd['place_id']) ? $pd['place_id'] : null;
    else error_log('[addAction] place lookup non-JSON: '.substr($resp,0,200));
  } else if ($cerr) {
    error_log('[addAction] place lookup error: '.$cerr);
  }
}

// Prefer client-sent place id if server lookup failed
...
$action = array(
  ...
  'actionLocationID'    => null,              // will fill via snap below
...
// Snap to known locations from data/locations if within 150m
