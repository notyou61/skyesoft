...
function generateZoningReport($prompt, &$conversation) {
    ...
    if ($normalizedJurisdiction) {
        ...
        if (!$primaryZoning) {
            $primaryZoning = "UNKNOWN";
            $baseDisclaimers[] = $disclaimersConfig['Zoning Report']['unsupportedJurisdiction'][0];
        }
        ...
        if ($normalizedJurisdiction === "GILBERT" && $primaryZoning === "PF/I (Public Facility/Institutional)") {
            ...
            // If no HVC in parcels, try offset (~100m east)
            if ($primaryZoning === "PF/I (Public Facility/Institutional)" && $latitude && $longitude) {
                $offsetLat = $latitude;
                $offsetLon = $longitude + 0.0009; // ~100m east
                $offsetZoning = getJurisdictionZoning($normalizedJurisdiction, $offsetLat, $offsetLon, null);
                if ($offsetZoning === "HVC (Heritage Village Center District)") {
                    $primaryZoning = $offsetZoning;
                    $baseDisclaimers[] = "⚠️ Address point zoning adjusted to match nearby coordinates due to possible geocoder offset.";
                }
            }
        }
    }
